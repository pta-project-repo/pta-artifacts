...
SWITCH_INFO_FILE = os.path.expandvars("$P4_PROJECT_DIR/ver_src/.sdnet_switch_info.dat")
...
if (len(PX_CAM_TABLES) > 0):
    ...
    libcam=cdll.LoadLibrary(os.path.expandvars('$P4_PROJECT_DIR/ver_sw/CLI/libcam.so'))
...
if (len(PX_TCAM_TABLES) > 0):
    ...
    libtcam=cdll.LoadLibrary(os.path.expandvars('$P4_PROJECT_DIR/ver_sw/CLI/libtcam.so'))
...
if (len(PX_LPM_TABLES) > 0):
    ...
    liblpm=cdll.LoadLibrary(os.path.expandvars('$P4_PROJECT_DIR/ver_sw/CLI/liblpm.so'))
...
TABLE_DEFINES_FILE = os.path.expandvars("$P4_PROJECT_DIR/ver_sw/CLI/ver_SimpleSumeSwitch_table_defines.json")
...
### -
def table_cam_clean_entries(table_name):
    if not check_valid_cam_table_name(table_name):
        return
        
    tableSize = table_cam_get_size(table_name)
    for x in range(0, tableSize):
        table_cam_delete_entry(table_name, [x])
